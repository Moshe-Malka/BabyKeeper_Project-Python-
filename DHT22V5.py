...
class sensor:
    ...
    '''
    publish a temperature reading if it is changed. (first reading is 0)
    '''
    def pubChangedTemperature():
      if s.temperature() != currTemperature :
          publish.single("babykeeper/temperature",s.temperature(),0,False,
                     hostname="broker.mqttdashboard.com", port=1883)
          currTemperature=s.temperature()
          
    '''
    publish a humidity reading if it is changed. (first reading is 0)
    '''
    def pubChangedHumidity():
      if s.humidity() != currHumidity :
          publish.single("babykeeper/humidity",s.humidity(),0,False,
                     hostname="broker.mqttdashboard.com", port=1883)
